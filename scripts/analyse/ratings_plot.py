...
def plot_ratings(
    path_to_heat_data: str, path_to_transport_data: str, path_to_plot: str
):
    averages = pd.concat([
    ...
    (
        facetted.configure(font="Lato", background="none")
...
def read_average_ratings(path_to_data: str, sector_name: str) -> pd.DataFrame:
    ...
    df["Emission attribution"] = (
        df.BY.str.split("/").map(lambda row: row[1]).str.capitalize()
    )
    return (
        df.groupby(["Climate change concern", "Emission attribution"])
        .estimate.mean()
        .rename("rating")
        .reset_index()
